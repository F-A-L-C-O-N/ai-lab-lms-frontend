...
const Login = ({ onNavigate, onAuthSuccess }) => {
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      setApiError(error.message || 'Social login failed');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              {!loginSuccess ? (
                <motion.div key="login-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="text-center mb-8">
                    <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-primary to-indigo-500 bg-clip-text text-transparent dark:from-indigo-400 dark:to-cyan-400">Welcome Back</h2>
                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Enter your credentials to access your AI Lab Learning Portal dashboard</p>
                  </div>
                  {apiError && (
