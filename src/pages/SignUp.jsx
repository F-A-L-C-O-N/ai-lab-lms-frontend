                ...
                <motion.div key="signup-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="relative mb-6">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-2 bg-transparent text-slate-500">Or sign up with email</span>
                    </div>
                  </div>
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                      <label className="flex items-start space-x-2 text-sm text-slate-600 dark:text-slate-400 cursor-pointer select-none">
                        <input type="checkbox" checked={termsAccepted} onChange={(e) => { setTermsAccepted(e.target.checked); if (errors.terms) setErrors(prev => ({ ...prev, terms: '' })); }} disabled={isLoading} className="mt-1 w-4.5 h-4.5 rounded border-slate-300 dark:border-slate-700 text-primary dark:text-indigo-500 focus:ring-primary dark:focus:ring-indigo-500 bg-white dark:bg-slate-800 transition" />
                        <span>I agree to the <a href="#" className="font-semibold text-primary dark:text-indigo-400 hover:underline">Terms of Service</a> and <a href="#" className="font-semibold text-primary dark:text-indigo-400 hover:underline">Privacy Policy</a></span>
                      </label>
                      {errors.terms && (<motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="flex items-center space-x-1 mt-1.5 text-red-500 text-xs"><AlertCircle size={12} /><span>{errors.terms}</span></motion.div>)}
                    </div>
